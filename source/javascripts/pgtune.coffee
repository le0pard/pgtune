class Pgtune
  ...
  _postgresSettings: =>
    ...
    if @dbVersion >= 9.5
      ...
      if cpuNum > 1
        parallelForWorkMem = Math.ceil(cpuNum / 2)
    ...
    # The other thing to consider is that there's no reason to run on the edge of available memory. If you do that, there's a very high risk the out-of-memory killer will come along and start killing PostgreSQL backends. Always leave a buffer of some kind in case of spikes in memory usage. So your maximum amount of memory available in work_mem should be ((RAM - shared_buffers) / (max_connections * 3 ) / (max_worker_processes / 2) / 2).
    workMem = (memoryInKB - gConfig['shared_buffers']) / (gConfig['max_connections'] * 3) / parallelForWorkMem / 2
    gConfig['work_mem'] = {
